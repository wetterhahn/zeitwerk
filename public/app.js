...
function renderOrders() {
  ...
  const detail = order ? `<div class="order-detail"><form id="order-detail-form" class="order-detail-form" data-order-id="${escapeHtml(order.id)}"><div class="order-detail-head"><div><p class="eyebrow">Auftragsdaten</p><h2>${escapeHtml(order.number)} · ${escapeHtml(order.name)}</h2></div><label class="switch"><input name="active" type="checkbox" ${order.active ? 'checked' : ''}><span>Aktiv</span></label></div><div class="order-fields"><label><span>Auftragsnummer *</span><input name="number" required value="${escapeHtml(order.number)}"></label><label><span>Bezeichnung *</span><input name="name" required value="${escapeHtml(order.name)}"></label><label><span>Anforderer</span><input name="requester" value="${escapeHtml(order.requester || '')}"></label><label class="description-field"><span>Beschreibung / Bemerkung</span><textarea name="description" rows="4" placeholder="Arbeiten, Hinweise und weitere Details">${escapeHtml(order.description || '')}</textarea></label></div></form><div class="hours-matrix card"><div class="matrix-title"><h3>Mitarbeiterstunden</h3><strong>${formatHours(orderHours(order.id))} Std. gesamt</strong></div><div class="order-hours-row matrix-head"><span>Mitarbeiter</span>${dayNames.map((day) => `<span>${day.slice(0, 2)}</span>`).join('')}<span>Summe</span></div>${employeeRows}</div></div>` : '<div class="empty-card card"><p>Lege links zuerst einen Auftrag an.</p></div>';
