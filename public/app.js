...
function masterEmployeeFor(weekEmployee) { return state.employees.find((employee) => String(employee.personnelNumber) === String(weekEmployee.id)); }
function visibleWeekEmployees() { return state.week?.employees.filter((employee) => masterEmployeeFor(employee)?.hiddenFromTracking !== true) || []; }
function ensureOrderEmployee(orderId, employeeId) { const order = orderById(orderId); if (!order) return; order.employeeIds = Array.isArray(order.employeeIds) ? order.employeeIds.map(String) : []; if (!order.employeeIds.includes(String(employeeId))) order.employeeIds.push(String(employeeId)); }
...
function renderEntry() {
  ...
  const visibleEmployees = visibleWeekEmployees();
  if (!visibleEmployees.length) return `<section class="page empty-page"><div class="empty-card card"><div class="empty-icon">👥</div><h2>Keine sichtbaren Mitarbeiter</h2><p>Blende in der Mitarbeiterverwaltung mindestens eine aktive Person für Erfassung und Übersichten ein.</p><button class="button primary" data-action="go-users">Mitarbeiter öffnen</button></div></section>`;
  const employee = visibleEmployees.find((item) => String(item.id) === String(state.selectedEmployeeId)) || visibleEmployees[0];
  ...
  const people = visibleEmployees.map((item) => `<button class="person ${String(item.id) === String(employee.id) ? 'active' : ''}" data-employee="${escapeHtml(item.id)}"><span class="avatar">${escapeHtml(initials(item.name))}</span><span><b>${escapeHtml(item.name)}</b><small>Personal-Nr. ${escapeHtml(item.id)}</small></span><i>›</i></button>`).join('');
  ...
  return `<section class="page"><div class="entry-layout"><aside class="people-card card"><div class="panel-title"><div><p class="eyebrow">Vollzugriff</p><h2>Alle Mitarbeiter</h2></div><span class="count">${visibleEmployees.length}</span></div><div class="people-list">${people}</div></aside><section><div class="employee-heading"><span class="avatar">${escapeHtml(initials(employee.name))}</span><div><p class="eyebrow">Aktive Erfassung</p><h2>${escapeHtml(employee.name)}</h2></div><span class="balance ${Math.abs(stats.difference) < .01 ? 'ok' : ''}">${Math.abs(stats.difference) < .01 ? 'Vollständig' : `${formatHours(Math.abs(stats.difference))} Std. offen`}</span></div><div class="day-list">${days}</div></section><aside class="summary-column"><div class="summary card"><p class="eyebrow">Wochensumme</p><strong class="big">${formatHours(stats.total)}</strong><span>Netto-Stunden</span><div class="rule"></div><dl><div><dt>Zugeordnet</dt><dd>${formatHours(stats.allocated)} Std.</dd></div><div><dt>Differenz</dt><dd>${formatHours(stats.difference)} Std.</dd></div></dl></div><div class="check ${Math.abs(stats.difference) < .01 ? 'ok' : ''}">${Math.abs(stats.difference) < .01 ? '✓ Arbeitszeit und Aufträge stimmen überein.' : '! Bitte die Auftragszuordnung prüfen.'}</div></aside></div></section>`;
}

function renderOverview() {
  if (!state.week) return renderEmpty();
  const employees = visibleWeekEmployees();
  const allStats = employees.map(employeeStats); const total = allStats.reduce((sum, item) => sum + item.total, 0); const allocated = allStats.reduce((sum, item) => sum + item.allocated, 0); const complete = allStats.filter((item) => Math.abs(item.difference) < .01).length;
  const rows = employees.map((employee, employeeIndex) => { const stats = allStats[employeeIndex]; return `<button class="overview-row person-row" data-employee="${escapeHtml(employee.id)}"><span class="overview-name"><i class="avatar">${escapeHtml(initials(employee.name))}</i><b>${escapeHtml(employee.name)}<small>Nr. ${escapeHtml(employee.id)}</small></b></span>${stats.totals.map((value) => `<span>${formatHours(value)}</span>`).join('')}<strong>${formatHours(stats.total)}</strong><span>${formatHours(stats.allocated)}</span><span class="pill ${Math.abs(stats.difference) < .01 ? 'ok' : ''}">${Math.abs(stats.difference) < .01 ? 'Fertig' : `${formatHours(Math.abs(stats.difference))} offen`}</span></button>`; }).join('');
  return `<section class="page"><div class="kpis"><div class="kpi card"><span>Teamstunden</span><strong>${formatHours(total)}</strong></div><div class="kpi card"><span>Zugeordnet</span><strong>${formatHours(allocated)}</strong></div><div class="kpi card"><span>Vollständig</span><strong>${complete} / ${employees.length}</strong></div></div><div class="table-card card"><div class="overview-row head"><span>Mitarbeiter</span>${dayNames.map((day) => `<span>${day.slice(0,2)}</span>`).join('')}<span>Woche</span><span>Aufträge</span><span>Status</span></div>${rows || '<div class="empty-card"><p>Alle Mitarbeiter sind ausgeblendet.</p></div>'}</div></section>`;
}

function orderHours(orderId) { return visibleWeekEmployees().reduce((sum, employee) => sum + employee.days.flatMap((day) => day.allocations).filter((item) => item.orderId === orderId).reduce((part, item) => part + Number(item.hours || 0), 0), 0); }
...
function renderOrders() {
  ...
  const visibleEmployees = visibleWeekEmployees();
  const assignedIds = new Set((order?.employeeIds || []).map(String));
  const assignedEmployees = order ? visibleEmployees.filter((employee) => assignedIds.has(String(employee.id))) : [];
  const availableEmployees = order ? visibleEmployees.filter((employee) => !assignedIds.has(String(employee.id))) : [];
  const employeeRows = order ? assignedEmployees.map((employee) => `<div class="order-hours-row"><span class="order-employee"><i class="avatar">${escapeHtml(initials(employee.name))}</i><b>${escapeHtml(employee.name)}<small>Nr. ${escapeHtml(employee.id)}</small></b></span>${dayNames.map((day, dayIndex) => `<label><span>${day.slice(0, 2)}</span><input class="order-hours-input" data-employee-id="${escapeHtml(employee.id)}" data-day-index="${dayIndex}" type="number" min="0" step="0.25" value="${escapeHtml(orderDayHours(employee, dayIndex, order.id) || '')}" aria-label="${escapeHtml(employee.name)} ${day} Stunden"></label>`).join('')}<strong>${formatHours(employee.days.reduce((sum, day, dayIndex) => sum + orderDayHours(employee, dayIndex, order.id), 0))}</strong><button class="icon-button remove-order-employee" data-employee-id="${escapeHtml(employee.id)}" title="Mitarbeiter aus Auftrag entfernen" aria-label="${escapeHtml(employee.name)} aus Auftrag entfernen">×</button></div>`).join('') : '';
  const employeePicker = availableEmployees.length ? `<div class="order-employee-picker"><select id="order-employee-select" aria-label="Mitarbeiter auswählen">${availableEmployees.map((employee) => `<option value="${escapeHtml(employee.id)}">${escapeHtml(employee.name)} · Nr. ${escapeHtml(employee.id)}</option>`).join('')}</select><button class="button secondary" id="add-order-employee" type="button">＋ Mitarbeiter hinzufügen</button></div>` : `<span class="all-assigned-note">${visibleEmployees.length ? 'Alle sichtbaren Mitarbeiter hinzugefügt' : 'Keine sichtbaren Mitarbeiter'}</span>`;
  const detail = order ? `<div class="order-detail"><form id="order-detail-form" class="order-detail-form" data-order-id="${escapeHtml(order.id)}"><div class="order-detail-head"><div><p class="eyebrow">Auftragsdaten</p><h2>${escapeHtml(order.number)} · ${escapeHtml(order.name)}</h2></div><label class="switch"><input name="active" type="checkbox" ${order.active ? 'checked' : ''}><span>Aktiv</span></label></div><div class="order-fields"><label><span>Auftragsnummer *</span><input name="number" required value="${escapeHtml(order.number)}"></label><label><span>Bezeichnung *</span><input name="name" required value="${escapeHtml(order.name)}"></label><label><span>Anforderer</span><input name="requester" value="${escapeHtml(order.requester || '')}"></label><label class="description-field"><span>Beschreibung / Bemerkung</span><textarea name="description" rows="4" placeholder="Arbeiten, Hinweise und weitere Details">${escapeHtml(order.description || '')}</textarea></label></div></form><div class="hours-matrix card"><div class="matrix-title"><div><h3>Mitarbeiterstunden</h3><p>Mitarbeiter einzeln hinzufügen und Stunden pro Tag eintragen.</p></div><strong>${formatHours(orderHours(order.id))} Std. gesamt</strong></div><div class="matrix-actions">${employeePicker}</div><div class="order-hours-row matrix-head"><span>Mitarbeiter</span>${dayNames.map((day) => `<span>${day.slice(0, 2)}</span>`).join('')}<span>Summe</span><span></span></div>${employeeRows || '<div class="matrix-empty">Noch keine Mitarbeiter zu diesem Auftrag hinzugefügt.</div>'}</div></div>` : '<div class="empty-card card"><p>Lege links zuerst einen Auftrag an.</p></div>';
...
function renderPeople() {
  const employeeCards = state.employees.map((employee) => {
    ...
    const isOwnAccount = account?.id === state.user.id;
    const visibilityTag = employee.hiddenFromTracking ? '<span class="role-tag hidden-tag">Ausgeblendet</span>' : '';
    return `<article class="user-card card ${employee.active ? '' : 'inactive'} ${employee.hiddenFromTracking ? 'tracking-hidden' : ''}"><div class="user-head"><span class="avatar">${escapeHtml(initials(employee.name))}</span><div><h3>${escapeHtml(employee.name)}</h3><p>Personal-Nr. ${escapeHtml(employee.personnelNumber)}${account ? ` · @${escapeHtml(account.username)}` : ''}</p></div>${visibilityTag}<span class="role-tag">${account ? 'Mit Anmeldung' : 'Ohne Anmeldung'}</span></div><form class="employee-edit employee-unified-form" data-employee-id="${escapeHtml(employee.id)}"><label><span>Name</span><input name="name" maxlength="120" value="${escapeHtml(employee.name)}" required></label><label><span>Personal-Nr.</span><input name="personnelNumber" maxlength="40" value="${escapeHtml(employee.personnelNumber)}" required></label><label class="switch login-toggle"><input name="loginEnabled" type="checkbox" ${account ? 'checked' : ''} ${isOwnAccount ? 'disabled' : ''}><span>Anmeldung mit Passwort</span></label><div class="login-fields" ${account ? '' : 'hidden'}><label><span>Benutzername</span><input name="username" minlength="3" maxlength="40" value="${escapeHtml(account?.username || '')}" ${account ? 'required' : ''}></label><label><span>${account ? 'Neues Passwort' : 'Passwort'}</span><input name="password" type="password" minlength="8" maxlength="200" placeholder="${account ? 'Unverändert' : 'Mindestens 8 Zeichen'}"></label></div><label class="switch visibility-toggle"><input name="hiddenFromTracking" type="checkbox" ${employee.hiddenFromTracking ? 'checked' : ''}><span>Aus Erfassung und Übersichten ausblenden</span></label><label class="switch"><input name="active" type="checkbox" ${employee.active ? 'checked' : ''} ${isOwnAccount ? 'disabled' : ''}><span>Aktiv</span></label><div class="employee-actions"><button class="button secondary" type="submit">Speichern</button><button class="button danger delete-employee" type="button" data-employee-id="${escapeHtml(employee.id)}" data-employee-name="${escapeHtml(employee.name)}" ${isOwnAccount ? 'disabled title="Der eigene Mitarbeiter kann nicht gelöscht werden"' : ''}>Löschen</button></div></form></article>`;
  }).join('');
  return `<section class="page people-admin"><div class="management-layout"><div class="management-create card"><p class="eyebrow">Standard: ohne Anmeldung</p><h2>Mitarbeiter anlegen</h2><p>Alle Personen werden hier angelegt. Nur wenn der Haken gesetzt wird, sind zusätzlich Benutzername und Passwort erforderlich.</p><form id="employee-form" class="stack-form"><label><span>Name</span><input name="name" maxlength="120" required></label><label><span>Personalnummer</span><input name="personnelNumber" maxlength="40" required></label><label class="switch login-toggle"><input name="loginEnabled" type="checkbox"><span>Anmeldung mit Passwort</span></label><div class="login-fields" hidden><label><span>Benutzername</span><input name="username" minlength="3" maxlength="40"></label><label><span>Passwort</span><input name="password" type="password" minlength="8" maxlength="200"></label></div><label class="switch visibility-toggle"><input name="hiddenFromTracking" type="checkbox"><span>Aus Erfassung und Übersichten ausblenden</span></label><button class="button primary" type="submit">Mitarbeiter anlegen</button></form></div><div class="management-list"><p class="eyebrow">Eine gemeinsame Liste</p><h2>${state.employees.length} Mitarbeiter</h2><p class="muted-copy employee-history-note">Ausgeblendete Personen können sich weiterhin anmelden und behalten Vollzugriff. Sie erscheinen nur nicht in Erfassung, Wochenübersicht und Auftragszuordnung. Beim Löschen bleiben bereits erfasste Wochen als Historie erhalten.</p><div class="user-list">${employeeCards || '<div class="empty-card card"><p>Noch keine Mitarbeiter vorhanden.</p></div>'}</div></div></div></section>`;
}
...
function bindContentEvents() {
  ...
  content.querySelector('[data-action="go-users"]')?.addEventListener('click', () => { state.view = 'users'; render(); });
  ...
  content.querySelector('#employee-form')?.addEventListener('submit', async (event) => {
    event.preventDefault(); const form = event.currentTarget; const data = Object.fromEntries(new FormData(form)); data.loginEnabled = form.elements.loginEnabled.checked; data.hiddenFromTracking = form.elements.hiddenFromTracking.checked; const button = form.querySelector('button'); button.disabled = true;
    try { await api('/api/employees', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }); await loadUsers(); if (state.week) await loadWeeks(state.week.id); showNotice(data.loginEnabled ? 'Mitarbeiter und Anmeldung wurden angelegt.' : 'Mitarbeiter wurde ohne Anmeldung angelegt.'); render(); }
  ...
  content.querySelectorAll('.employee-edit').forEach((form) => form.addEventListener('submit', async (event) => {
    event.preventDefault(); const data = Object.fromEntries(new FormData(form)); data.active = form.elements.active.checked; data.loginEnabled = form.elements.loginEnabled.checked; data.hiddenFromTracking = form.elements.hiddenFromTracking.checked; const button = form.querySelector('button'); button.disabled = true;
    try { await api(`/api/employees/${encodeURIComponent(form.dataset.employeeId)}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) }); await loadUsers(); if (state.week) await loadWeeks(state.week.id); showNotice('Mitarbeiter wurde aktualisiert.'); render(); }
    catch (error) { showNotice(error.message, true); button.disabled = false; }
  }));
  content.querySelectorAll('.delete-employee').forEach((button) => button.addEventListener('click', async () => {
    const name = button.dataset.employeeName;
    if (!window.confirm(`${name} wirklich löschen?\n\nStammdaten und ein vorhandener Zugang werden entfernt. Bereits erfasste Wochen bleiben erhalten.`)) return;
    button.disabled = true;
    try { const result = await api(`/api/employees/${encodeURIComponent(button.dataset.employeeId)}`, { method: 'DELETE' }); await loadUsers(); showNotice(result.message); render(); }
    catch (error) { showNotice(error.message, true); button.disabled = false; }
  }));
  ...
  content.querySelector('#add-order-employee')?.addEventListener('click', () => { const select = content.querySelector('#order-employee-select'); if (!select?.value) return; ensureOrderEmployee(state.selectedOrderId, select.value); markDirty(); render(); });
  content.querySelectorAll('.remove-order-employee').forEach((button) => button.addEventListener('click', () => { const order = orderById(state.selectedOrderId); if (!order) return; order.employeeIds = (order.employeeIds || []).filter((id) => String(id) !== String(button.dataset.employeeId)); dayNames.forEach((day, dayIndex) => setOrderDayHours(button.dataset.employeeId, dayIndex, order.id, 0)); markDirty(); render(); }));
  content.querySelectorAll('.order-hours-input').forEach((input) => input.addEventListener('change', () => { ensureOrderEmployee(state.selectedOrderId, input.dataset.employeeId); setOrderDayHours(input.dataset.employeeId, Number(input.dataset.dayIndex), state.selectedOrderId, Math.max(0, Number(input.value || 0))); markDirty(); render(); }));
